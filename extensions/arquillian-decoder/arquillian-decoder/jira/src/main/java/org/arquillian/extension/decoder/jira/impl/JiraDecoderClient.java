...
import org.arquillian.extension.decoder.api.DecoderClient;
 ...
 *
 */
public class JiraDecoderClient implements DecoderClient<Jira, JiraDecoderStrategy>
{
    private JiraRestClient restClient;
    private JiraDecoderStrategy jiraDecoderStrategy;
    private JiraDecoderConfiguration jiraDecoderConfiguration;

    private int JIRA_BUILD_NUMBER = 0;

    public void setConfiguration(JiraDecoderConfiguration jiraDecoderConfiguration)
    {
        Validate.notNull(jiraDecoderConfiguration, "Jira Decoder configuration must be specified.");
        this.jiraDecoderConfiguration = jiraDecoderConfiguration;
    }
    ...
    public ExecutionDecision resolve(final Jira annotation)
    {
        ...
        Validate.notNull(jiraDecoderStrategy, "Decoder strategy must be specified. Have you already called setDecoderStrategy()?");
        ...
        return jiraDecoderStrategy.annotation(annotation).issue(jiraIssue).resolve();
    }
    ...
    @Override
    public void setDecoderStrategy(JiraDecoderStrategy jiraDecoderStrategy)
    {
        Validate.notNull(jiraDecoderStrategy, "Jira decoder strategy must be specified.");
        this.jiraDecoderStrategy = jiraDecoderStrategy;
    }
    ...
    {
        Validate.notNull(jiraDecoderConfiguration, "Jira Decoder configuration must be set.");

        return String.format(jiraDecoderConfiguration.getClosingMessage(), jiraDecoderConfiguration.getUsername());
    }
